...
module.exports = function (app) {
  ...
  const apiBase = 'https://Takanashi.dpdns.org/anime/alqanime';
  ...
  app.get('/anime/alqanime/hangat', async (req, res) => {
    try {
      ...
      res.json({ status: true, creator: "Takanashi", result });
    } catch (err) {
  ...
  app.get('/anime/alqanime/download', async (req, res) => {
    ...
    try {
      ...
      res.json({ status: true, creator: "Takanashi", result: downloadLinks });
    } catch (err) {
      res.status(500).json({ status: false, message: err.message });
    }
  });
