...
module.exports = function (app) {
  ...
  app.get('/anime/kompi/home', async (req, res) => {
    ...
    try {
      ...
      res.json({ status: true, creator: 'Takanashi', result: scrapedData });
    } catch (err) {
      res.status(500).json({ status: false, creator: 'Takanashi', message: err.message });
    }
  });
  ...
  app.get('/anime/kompi/search', async (req, res) => {
    const q = req.query.q;
    if (!q) return res.status(400).json({ status: false, creator: 'Takanashi', message: 'Parameter q wajib diisi' });
    ...
    try {
      ...
      res.json({ status: true, creator: 'Takanashi', result: searchResults });
    } catch (err) {
      res.status(500).json({ status: false, creator: 'Takanashi', message: err.message });
    }
  });
...
app.get('/anime/kompi/detail', async (req, res) => {
    ...
    if (!url) return res.status(400).json({ status: false, creator: 'Takanashi', message: 'Parameter url wajib diisi' });
    ...
    try {
      ...
      res.json({ status: true, creator: 'Takanashi', result: animeData });
    } catch (err) {
      res.status(500).json({ status: false, creator: 'Takanashi', message: err.message });
    }
  });
