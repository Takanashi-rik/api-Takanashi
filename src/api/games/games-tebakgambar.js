...
module.exports = function (app) {
    app.get("/games/tebakgambar", async (req, res) => {
        try {
            ...
            if (!data || !Array.isArray(data) || data.length === 0) {
                return res.status(500).json({
                    status: false,
                    creator: "Takanashi",
                    message: "Data tebakgambar tidak valid atau kosong.",
            ...
            if (error.response) {
                return res.status(error.response.status).json({
                    status: false,
                    creator: "Takanashi",
                    message: "Gagal mengambil data tebakgambar dari GitHub.",
